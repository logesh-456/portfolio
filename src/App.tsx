...
const Contact = () => {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) { setResult('Form Submitted Successfully!'); (event.target as HTMLFormElement).reset(); }
      else setResult(data.message);
    } catch { setResult('Submit failed. Please try again.'); }
  };
  return (
    <section id="contact" className="py-24">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-16">
          <div>
            <h2 className="text-4xl md:text-5xl font-bold mb-6">Let's build something <span className="gradient-text">amazing</span> together.</h2>
            <p className="text-neutral-400 mb-10 text-lg">Have a project in mind? Reach out and let's discuss how we can help your business grow.</p>
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 glass rounded-2xl flex items-center justify-center text-blue-400"><Github size={24} /></div>
                <div><div className="text-sm text-neutral-500">GitHub</div><a href="https://github.com/logesh-456" className="hover:text-blue-400 transition-colors">Visit</a></div>
              </div>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 glass rounded-2xl flex items-center justify-center text-blue-400"><Linkedin size={24} /></div>
                <div><div className="text-sm text-neutral-500">LinkedIn</div><a href="https://www.linkedin.com/in/logesh-a-a84303364" className="hover:text-blue-400 transition-colors">Visit</a></div>
              </div>
            </div>
          </div>
            ...
            <form className="space-y-6" onSubmit={onSubmit}>
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input name="name" type="text" required className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors" placeholder="Logesh" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-neutral-400">Email</label>
                  <input name="email" type="email" required className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors" placeholder="you@email.com" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-neutral-400">Message</label>
                <textarea name="message" rows={4} required className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors" placeholder="Tell me about your project…" />
              </div>
              <button type="submit" className="w-full py-4 rounded-2xl bg-blue-600 hover:bg-blue-700 font-bold transition-all glow">Send Message</button>
              {result && <p className={`text-center text-sm mt-4 ${result.includes('Successfully') ? 'text-green-400' : 'text-neutral-400'}`}>{result}</p>}
            </form>
          </div>
        </div>
      </div>
